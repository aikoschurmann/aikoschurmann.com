...
- Apply the three fundamental regular operations: union ($\cup$), concatenation, and Kleene star ($^*$)
...
Recall from Chapter 1 that the lexer is the first phase of compilation. It receives raw source code as a stream of characters and must identify token boundaries. Given the input:
...
Modern compilers maintain a clean separation between two phases:

| Phase | Input | Model | Handles |
|-------|-------|-------|---------|
| Lexer | Characters | Regular languages | Token boundaries, pattern matching |
| Parser | Tokens | Context-free grammars | Nested structure, syntax trees |
...
We build from the ground up. Every concept in this section will be used throughout the rest of the course.
...
> **Critical distinction**: $\epsilon$ is the empty string (zero symbols). The empty set $\emptyset$ contains no strings at all. These are completely different objects that are easy to conflate.
...
Before defining operations on collections of strings, we need operations on individual strings.

**Concatenation**: For strings $x$ and $y$, the concatenation $xy$ appends $y$ to $x$.

- If $x = abc$ and $y = de$, then $xy = abcde$
- $|xy| = |x| + |y|$
- $\epsilon x = x\epsilon = x$ for all $x$ (identity element)
- $(xy)z = x(yz)$ (associative), but $xy \ne yx$ in general (not commutative)
...
Since $\Sigma^*$ is infinite, languages can be finite, infinite, or empty:
...
The three operations in this section are the building blocks of everything that follows. Learn them well.
...
**Example**:
- $L_1 = \{a, aa\}$, $\quad L_2 = \{b, bb\}$
- $L_1L_2 = \{ab,\ abb,\ aab,\ aabb\}$

Two special cases are worth memorizing:

- $L \cdot \{\epsilon\} = \{\epsilon\} \cdot L = L$ (the language $\{\epsilon\}$, not the empty set, is the identity)
...
The Kleene star is the most powerful and distinctive operation on languages.

$$L^* = \bigcup_{i=0}^{\infty} L^i = L^0 \cup L^1 \cup L^2 \cup \cdots$$

where $L^0 = \{\epsilon\}$, $L^1 = L$, $L^2 = LL$, and so on.
...
**Takeaway**: $\{a\}^*\{b\}^* \subsetneq (\{a\} \cup \{b\})^*$. The placement of $^*$ matters.
...
$$\{ab^n \mid n \ge 0\} \cup \{aab^n \mid n \ge 0\}$$

**Example D**: $\{ab, ba\}^2$

$L^2 = LL = \{ab, ba\}\{ab, ba\} = \{abab,\ abba,\ baab,\ baba\}$
...
**Non-example**: The language $L = \{a^n b^n \mid n \ge 0\}$ is **not** regular. Membership requires matching two unbounded counts — the number of $a$'s and the number of $b$'s — and finite memory cannot enforce that equality for arbitrarily large $n$. We will prove this rigorously in Section 2.8.

### 2.5.2 Regular Expressions: Notation for Regular Languages

Writing out set-theoretic constructions like $\{a\}(\{a\}\cup\{b\})^*$ is cumbersome. Regular expressions are a compact notation for the same thing.
...
- $(r \mid s)$ — alternation (union)
- $(rs)$ — concatenation
- $(r^*)$ — Kleene star

**Semantics**: Each expression $r$ denotes a language $L(r)$:
...
This lets us omit most parentheses. Here are four worked examples showing how precedence affects meaning:

| Regex | Parsed as | Language | In English |
|-------|-----------|----------|------------|
| $ab^*c$ | $a(b^*)c$ | $\{ac, abc, abbc, \ldots\}$ | $a$, any number of $b$'s, then $c$ |
| $a \mid b^*$ | $a \mid (b^*)$ | $\{a, \epsilon, b, bb, \ldots\}$ | Either exactly $a$, or any run of $b$'s |
| $(a \mid b)^*$ | — | $\{a,b\}^*$ | All strings over $\{a,b\}$ |
| $a^*b^*$ | $(a^*)(b^*)$ | $\{a^n b^m \mid n,m \ge 0\}$ | All $a$'s before all $b$'s |

### 2.5.3 Extended Notation

The pure definition of regular expressions is minimal by design. In practice, we use convenient shorthands. None of these add expressive power — they are all syntactic sugar.

| Notation | Meaning | Example |
|----------|---------|---------|
| `r+` | `rr*` — one or more | `a+` = $\{a, aa, aaa, \ldots\}$ |
| `r?` | `(r\|ε)` — zero or one | `ab?c` = $\{ac, abc\}$ |
...
**Theorem**: If $L_1$ and $L_2$ are regular, then so are:

1. $L_1 \cup L_2$ — union
2. $L_1 L_2$ — concatenation
3. $L_1^*$ — Kleene star
4. $L_1 \cap L_2$ — intersection
5. $\overline{L_1} = \Sigma^* \setminus L_1$ — complement
6. $L_1 \setminus L_2 = L_1 \cap \overline{L_2}$ — difference
7. $L_1^R = \{w^R \mid w \in L_1\}$ — reversal

Properties 1–3 follow directly from the definition of regular languages. The others require construction arguments:

**Intersection** uses a *product automaton*: given two DFAs for $L_1$ and $L_2$, build a new DFA whose states are pairs $(q_1, q_2)$ with one component from each machine. Accept when both components are accepting. (We will formalize this in Chapter 3.)
...
- $L_2 = \Sigma^* \text{goto} \Sigma^*$ — strings containing `goto` — regular (concatenation of a constant with $\Sigma^*$ on both sides)
...
Regular languages are powerful, but they cannot express everything. This section gives us a precise tool for proving that specific languages are *not* regular.

### 2.7.1 Intuition

A DFA has a finite number of states — say $p$. If you feed it a string of length $\ge p$, by the **pigeonhole principle** it must visit some state twice. That means a segment of the input was processed in a loop. Any language that cannot tolerate repeating that segment is not regular.

### 2.7.2 The Pumping Lemma

**Theorem (Pumping Lemma)**: If $L$ is a regular language, then there exists an integer $p \ge 1$ (the *pumping length*) such that every string $w \in L$ with $|w| \ge p$ can be written as $w = xyz$ where:

1. $|y| > 0$ — the pumped portion is nonempty
2. $|xy| \le p$ — the split happens near the start
3. For all $i \ge 0$: $xy^iz \in L$ — pumping $y$ any number of times stays in $L$

The key phrase in condition 3 is *for all $i \ge 0$*. In particular, pumping zero times ($i = 0$) gives $xz$, which must also be in $L$.
...
3. Consider **all** valid splits $w = xyz$ satisfying conditions 1 and 2
4. Show that **for each** such split, some $i$ gives $xy^iz \notin L$
...
By the pumping lemma, $w = xyz$ with $|y| > 0$ and $|xy| \le p$.

Since $|xy| \le p$ and the first $p$ characters of $w$ are all $a$'s, both $x$ and $y$ consist entirely of $a$'s. So $y = a^k$ for some $k \ge 1$.

Now pump zero times: $xy^0z = xz$. This string has $p - k$ copies of $a$ and $p$ copies of $b$. Since $k \ge 1$, we have $p - k < p$, so $xz \notin L$.

This contradicts condition 3. Therefore $L$ is not regular. $\blacksquare$

### 2.7.5 Example: Balanced Parentheses

**Claim**: $L = \{(\,^n )\,^n \mid n \ge 0\}$ is not regular.

**Proof**: Choose $w = (\,^p)\,^p$. The argument is identical to the previous example: any valid split puts all of $y$ in the opening-parenthesis prefix, pumping produces a mismatch, contradiction. $\blacksquare$
...
| $\{ww \mid w \in \{a,b\}^*\}$ | Cannot remember the first half |
...
**What comes next**: Regular expressions tell us *what* to recognize. We still need an *operational* model — a machine that reads characters and decides acceptance. That is the subject of Chapter 3, where we will meet deterministic and nondeterministic finite automata and prove that they recognize exactly the regular languages.
...
**8.** Prove that if $L$ is regular, then $L^R = \{w^R \mid w \in L\}$ is also regular. (Hint: use induction on the structure of the regular expression for $L$, or argue via automata closure.)
...
**12.** Prove that the class of regular languages is closed under the following operation: $\text{HALF}(L) = \{x \mid \exists y : |x| = |y| \text{ and } xy \in L\}$. (Hint: this is non-trivial and requires an automaton construction argument. Think about what information you need to track.)